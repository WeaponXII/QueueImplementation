RelationalTable::RelationalTable(double loadThreshold = 0.5): loadThreshold(loadThreshold){
    capacity = 8;
...
void RelationalTable::addRow(const string values[], int size){
    int myHash = hash<string>() (values[0]) & (capacity - 1);
    int val_iterator = 0;
...
void RelationalTable::setValue(const string& pkValue, const string& fieldName, const string& fieldValue){
    int myHash = hash<string>()(pkValue);
    bool hashing = true;
    int quad_collider = 0;
    while(hashing){
        int newHash = (myHash + quad_collider^2) % capacity;
